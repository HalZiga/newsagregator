...
async def initialize_database(db: AsyncSession):
    ...
    for role_enum_member in RoleEnum:
        existing_role_result = await db.execute(select(RoleModel).where(RoleModel.name.is_(role_enum_member)))
    ...
    result = await db.execute(select(UserModel).where(UserModel.login.is_(admin_username)))
    existing_user = result.unique().scalars().one_or_none()

    if not existing_user:
        admin_role_result = await db.execute(select(RoleModel).where(RoleModel.name.is_(RoleEnum.Admin)))
