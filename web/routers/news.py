...
from web.services.news_service import (
    create_news_service,
    delete_news_service,
    get_all_news_authorized_service,
...
async def create_news(
    ...
    new_news = await create_news_service(
        news_data=news_data, db=db, current_user=current_user
    )
...
async def get_published_news(db: AsyncSession = Depends(get_db)):
    ...
    return [
        News.model_validate({**item.__dict__, "author": item.created_by.login})
...
async def get_news_by_id(
    ...
    logger.info(
        "Пользователь '%s' запрашивает новость с ID: %d",
    ...
    if bool(current_user):
        ...
        is_author = (
            bool(current_user) and news_data.created_by_user_id == current_user.id
        )
...
async def update_news(
    ...
    current_user: UserModel = Depends(
        role_required(
            [RoleEnum.Admin.value, RoleEnum.Moderator.value, RoleEnum.Author.value]
        )
    ),
):
    """
    ...
    updated_news = await update_news_service(
        news_id=news_id, news_data=news_data, db=db, current_user=current_user
    )
...
async def publish_news(
    ...
    """
    published_news = await publish_news_service(
        news_id=news_id, db=db, current_user=current_user
    )
